...
function createOrgs() {
  ...
  if [ "$CRYPTO" == "cryptogen" ]; then
    ...
    infoln "Creating India Identities"

    set -x
    cryptogen generate --config=./organizations/cryptogen/crypto-config-india.yaml --output="organizations"
    res=$?
    { set +x; } 2>/dev/null
    if [ $res -ne 0 ]; then
    ...
    infoln "Creating Australia Identities"

    set -x
    cryptogen generate --config=./organizations/cryptogen/crypto-config-australia.yaml --output="organizations"
    res=$?
    { set +x; } 2>/dev/null
    if [ $res -ne 0 ]; then
      fatalln "Failed to generate certificates..."
  ...
  while :
    do
      if [ ! -f "organizations/fabric-ca/india/tls-cert.pem" ]; then
        sleep 1
      else
        break
      fi
  ...
  infoln "Generating CCP files for India and Australia"
...
# call the script to create the channel, join the peers of india and australia,
...
function networkDown() {
  # stop org3 containers also in addition to india and australia, in case we were running sample to add org3
  ...
  if [ "$MODE" != "restart" ]; then
    ...
    docker run --rm -v $(pwd):/data busybox sh -c 'cd /data && rm -rf organizations/fabric-ca/india/msp organizations/fabric-ca/india/tls-cert.pem organizations/fabric-ca/india/ca-cert.pem organizations/fabric-ca/india/IssuerPublicKey organizations/fabric-ca/india/IssuerRevocationPublicKey organizations/fabric-ca/india/fabric-ca-server.db'
    docker run --rm -v $(pwd):/data busybox sh -c 'cd /data && rm -rf organizations/fabric-ca/australia/msp organizations/fabric-ca/australia/tls-cert.pem organizations/fabric-ca/australia/ca-cert.pem organizations/fabric-ca/australia/IssuerPublicKey organizations/fabric-ca/australia/IssuerRevocationPublicKey organizations/fabric-ca/australia/fabric-ca-server.db'
